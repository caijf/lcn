function dataTest(data) {
  it('检查数据', () => {
    expect(data.length).toBe(3218);
    expect(data[0]).toEqual({ code: '110000', name: '北京市' });
    expect(data[data.length - 1]).toEqual({ code: '820000', name: '澳门特别行政区' });
    // 包含省
  ...
  it('部分省补充县级行政区划', () => {
    ...
    expect(data).toContainEqual({ code: '659000', name: '自治区直辖县级行政区划' });
  });

  it('海南省三沙市补充区级数据', () => {
    expect(data).toContainEqual({ code: '460321', name: '西沙群岛' });
    expect(data).toContainEqual({ code: '460322', name: '南沙群岛' });
    expect(data).toContainEqual({ code: '460323', name: '中沙群岛的岛礁及其海域' });
  });
